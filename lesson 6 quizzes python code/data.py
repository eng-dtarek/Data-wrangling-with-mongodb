...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for innertag in element:
            if node['type']=="way":
                if innertag.tag=='nd':
                     node_refs.append(innertag.attrib['ref'])
                node['node_refs']=node_refs
            if innertag.tag=='tag':
                if innertag.attrib['k'].startswith('addr:'):                   
                    addr=innertag.attrib['k'].replace('addr:','')
                    if addr.find(':') >0:
                        continue
                    else:
                        address[addr]=innertag.attrib['v']
                        node['address']=address
                else:
                    node[innertag.attrib['k']]=innertag.attrib['v']
